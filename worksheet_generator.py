"""
=============================================================================
  WORKSHEET GENERATOR  —  worksheet_generator.py
=============================================================================
  Generates personalised LaTeX worksheets from student profiles.

  Flow:
    1. AI plans the worksheet (question count, difficulty, topics)
    2. AI generates each question as complete .tex + mark scheme JSON
    3. Python assembles the full LaTeX project
    4. pdflatex compiles → PDF
    5. If compile fails → AI fixes the error → retry (up to 3x)
    6. Returns: student PDF + mark scheme JSON (for AI grader)
=============================================================================
"""

import os
import re
import json
import uuid
import shutil
import subprocess
import traceback
from pathlib import Path
from typing import Optional
from datetime import date

from google import genai
from google.genai import types
from pydantic import BaseModel, Field
from typing import List
from dotenv import load_dotenv

load_dotenv()

# ── Config ────────────────────────────────────────────────────────────────────
TEMPLATE_DIR  = Path(__file__).parent / "template"
JOBS_DIR      = Path(__file__).parent / "worksheet_jobs"
MODEL         = "gemini-2.5-flash"
MAX_RETRIES   = 3
JOBS_DIR.mkdir(exist_ok=True)

# ── Full macro reference injected into every AI prompt ───────────────────────
# This tells the AI exactly what macros are available so it never invents its own.
MACRO_REFERENCE = r"""
AVAILABLE LATEX MACROS (use ONLY these for answer boxes):

QUESTION STRUCTURE:
  \begin{question} ... \end{question}     — wraps each question, auto-numbers, auto-totals marks
  \partq{a}  \partq{b}  etc.             — sub-part labels inside a question

ANSWER MACROS (always place at end of each part, right-aligned):
  \answerplain{marks}                    — dotted line only         e.g. \answerplain{3}
  \answerunit{unit}{marks}               — dotted line + unit       e.g. \answerunit{cm}{2}
  \answerprefix{prefix}{marks}           — prefix + dotted line     e.g. \answerprefix{\$}{2}
  \answereq{variable}{marks}             — x = dotted line          e.g. \answereq{x}{2}
  \answerequnit{variable}{unit}{marks}   — x = dotted line + unit   e.g. \answerequnit{v}{m/s}{3}
  \answerlines{num_lines}{marks}         — ruled lines for written explanation
  \answercoord{marks}                    — coordinate pair ( . , . )
  \answermarks{marks}                    — marks only, no answer line (use for show-that questions)
  \answermcq{A}{B}{C}{D}{marks}          — 4-option MCQ with tickboxes

SPACING:
  \vspace{Xcm}    — vertical space for working. Use generously (4cm minimum per part).

MATHS:
  Inline:  $...$   e.g. $x^2 + 3x - 2 = 0$
  Display: \[ ... \]  for standalone equations

TIKZ DIAGRAMS:
  Use standard tikz. Available libraries: angles, quotes, calc, 3dplot.
  Always wrap diagrams in \begin{center}...\end{center}.
  Use [scale=0.8] or similar to ensure diagrams fit the page width.

DO NOT invent new macros. DO NOT use \begin{exam} or similar — it does not exist.
"""

# ── Sample questions injected as few-shot examples ────────────────────────────
SAMPLE_QUESTIONS = r"""
EXAMPLE QUESTION FILES (study these for style and structure):

--- EXAMPLE: Geometry with tikz diagram (q1.tex style) ---
\begin{question}
\noindent The diagram shows two right-angled triangles, $ABD$ and $CDE$.
$ADC$ and $BDE$ are straight lines intersecting at point $D$.

\begin{center}
\begin{tikzpicture}[scale=0.6]
    \draw[thick] (-6, 0) node[below]{$A$} -- (8.5, 0) node[below]{$C$};
    \draw[thick] (-6, 8) node[above]{$B$} -- (3, -4) node[below]{$E$};
    \draw[thick] (-6, 0) -- (-6, 8);
    \draw[thick] (3, -4) -- (8.33, 0);
    \draw (-5.6, 0) -- (-5.6, 0.4) -- (-6, 0.4);
    \draw (2.76, -3.68) -- (3.08, -3.44) -- (3.32, -3.76);
    \node at (0.3, 0.4) {$D$};
    \node at (-6.5, 4) {$8$ cm};
    \node at (-3, -0.5) {$6$ cm};
    \node at (5, 0.5) {$12.5$ cm};
\end{tikzpicture}
\end{center}

\noindent $AB = 8$ cm, $AD = 6$ cm, $CD = 12.5$ cm.
Work out the length of $CE$.
\vspace{4.5cm}
\begin{flushright}
    \answerunit{cm}{3}
\end{flushright}
\end{question}

--- EXAMPLE: Multi-part algebra (q3.tex style) ---
\begin{question}
$y$ is inversely proportional to $x^n$, where $n$ is an integer.

The table shows some values of $x$ and $y$.

\begin{center}
\begin{tabular}{c|ccc}
$x$ & 3 & 6 & $q$ \\
\hline
$y$ & 40 & 5 & 0.625 \\
\end{tabular}
\end{center}

\begin{enumerate}
    \item[(a)] Find the value of $n$.
    \vspace{4cm}
    \answereq{n}{2}

    \item[(b)] Find a formula for $y$ in terms of $x$.
    \vspace{2cm}
    \answereq{y}{2}

    \item[(c)] Find the value of $q$.
    \vspace{1.5cm}
    \answereq{q}{2}
\end{enumerate}
\end{question}

--- EXAMPLE: Word problem (q4.tex style) ---
\begin{question}
A farmer buys 749 sheep for a total cost of $C$.\\
He sells 700 of the sheep for $C$.\\
The farmer then sells the remaining 49 sheep at the same price per sheep.\\
Work out the percentage profit that the farmer makes.
\vspace{4cm}
\answerplain{2}
\end{question}

--- EXAMPLE: Table with probability ---
\begin{question}
A six-sided dice is rolled once. The table shows the probabilities of some events.

\begin{center}
\renewcommand{\arraystretch}{1.6}
\setlength{\arrayrulewidth}{1.2pt}
\begin{tabular}{|>{\centering\arraybackslash}m{2.8cm}|
                  >{\centering\arraybackslash}m{2.8cm}|
                  >{\centering\arraybackslash}m{2.8cm}|}
\hline
Event & Even number & Prime number \\ \hline
Probability & $\dfrac{1}{2}$ & $\dfrac{1}{3}$ \\ \hline
\end{tabular}
\end{center}

If the dice is rolled 90 times, estimate the number of times it will show a prime number.
\vspace{4cm}
\answerplain{2}
\end{question}
"""


# ============================================================================
# SECTION 1 — PYDANTIC SCHEMAS
# ============================================================================

class QuestionPlan(BaseModel):
    number:           int
    topic_aspect:     str  = Field(description="Specific aspect of the topic, e.g. 'SOH CAH TOA — finding a side'")
    difficulty:       str  = Field(description="easy / medium / hard")
    question_type:    str  = Field(description="single_part / multi_part / word_problem / diagram / table / mcq / show_that")
    marks:            int
    has_diagram:      bool
    misconception_targeted: Optional[str] = Field(default=None)
    notes:            str  = Field(default="", description="Brief generation note for the AI")

class WorksheetPlan(BaseModel):
    title:                   str
    num_questions:           int
    total_marks_estimate:    int
    difficulty_distribution: str
    pedagogical_rationale:   str  = Field(description="Why this structure suits this student")
    questions:               List[QuestionPlan]

class GeneratedQuestion(BaseModel):
    number:       int
    tex_content:  str  = Field(description="Complete LaTeX content for this question, ready to \input{}")
    marks:        int
    answer:       str  = Field(description="The correct final answer(s)")
    mark_scheme:  str  = Field(description="Mark scheme in Edexcel format: M1 for..., A1 for..., etc.")
    topic_aspect: str


# ============================================================================
# SECTION 2 — LATEX ASSEMBLY
# ============================================================================

def build_config_tex(meta: dict) -> str:
    """Generate config.tex content from worksheet metadata."""
    return rf"""
% Auto-generated by Cloud LearnX Worksheet Generator
\newcommand{{\examDate}}{{{meta['date']}}}
\newcommand{{\examBoard}}{{{meta['board']}}}
\newcommand{{\examSubject}}{{{meta['subject']}}}
\newcommand{{\examPaper}}{{{meta['paper_title']}}}
\newcommand{{\examTier}}{{{meta['tier']}}}
\newcommand{{\examCode}}{{{meta['code']}}}
\newcommand{{\examTime}}{{{meta['time']}}}
\newcommand{{\totalMarks}}{{\total{{totalmarks}}}}
"""


def build_main_tex(num_questions: int, title: str) -> str:
    """Generate main.tex that inputs all question files."""
    question_inputs = "\n".join([
        f"\\input{{questions/q{i}}}\n\\clearpage" if i < num_questions
        else f"\\input{{questions/q{i}}}"
        for i in range(1, num_questions + 1)
    ])

    return rf"""\documentclass[11pt]{{article}}
\input{{layout/packages}}
\input{{config}}
\input{{layout/macros}}

\renewcommand{{\familydefault}}{{\sfdefault}}

\renewcommand{{\marks}}[1]{{%
    \textbf{{(#1)}}%
    \addtocounter{{questionmarks}}{{#1}}%
    \addtocounter{{totalmarks}}{{#1}}%
}}

\newcounter{{totalmarks}}
\newcounter{{questionmarks}}
\newcounter{{questionNum}}
\regtotcounter{{totalmarks}}

\begin{{document}}
\input{{cover}}
\newpage

\booltrue{{drawborder}}
\newgeometry{{left=0.6in, right=0.6in, top=2cm, bottom=2cm}}
\pagestyle{{plain}}

\firstpageheader{{ALL}}

{question_inputs}

\paperTotal
\end{{document}}
"""


def build_cover_tex(student_name: str, worksheet_title: str) -> str:
    """Simplified worksheet cover (not an exam — no candidate boxes needed)."""
    return rf"""
\pagestyle{{empty}}
\begin{{center}}
    \vspace*{{2cm}}
    {{\Huge \textbf{{\examBoard}}}} \\[5mm]
    {{\LARGE \textbf{{\examSubject}}}} \\[3mm]
    {{\Large \examPaper}} \\[8mm]
    \begin{{tcolorbox}}[colback=white, colframe=black, arc=4mm, width=0.7\textwidth,
                       boxrule=1.5pt, top=4mm, bottom=4mm]
        \centering
        {{\large \textbf{{Student:}}}} {student_name} \\[3mm]
        {{\large \textbf{{Date:}}}} \examDate \\[3mm]
        {{\large \textbf{{Topic:}}}} \examTier
    \end{{tcolorbox}}
    \vspace{{5mm}}
    {{\large \textbf{{Instructions:}}}}\\[2mm]
    \begin{{itemize}}[leftmargin=3cm]
        \item Answer ALL questions.
        \item Show ALL working — method marks are awarded for correct steps.
        \item Write answers in the spaces provided.
    \end{{itemize}}
\end{{center}}
\newpage
"""


def assemble_latex_project(
    job_dir:        Path,
    meta:           dict,
    student_name:   str,
    generated_qs:   List[GeneratedQuestion],
    worksheet_plan: WorksheetPlan,
):
    """Write all .tex files into the job directory."""
    (job_dir / "layout").mkdir(exist_ok=True)
    (job_dir / "questions").mkdir(exist_ok=True)
    (job_dir / "assets").mkdir(exist_ok=True)

    # Copy layout files from template
    for f in ["packages.tex", "macros.tex"]:
        src = TEMPLATE_DIR / "layout" / f
        dst = job_dir / "layout" / f
        if src.exists():
            shutil.copy(src, dst)
        else:
            print(f"  ⚠️  Template file missing: {src} — worksheet may not compile correctly")

    # Copy assets (formula sheet, images etc.)
    assets_src = TEMPLATE_DIR / "assets"
    if assets_src.exists():
        for asset in assets_src.iterdir():
            shutil.copy(asset, job_dir / "assets" / asset.name)

    # Write generated files
    (job_dir / "config.tex").write_text(build_config_tex(meta))
    (job_dir / "cover.tex").write_text(build_cover_tex(student_name, meta['paper_title']))
    (job_dir / "main.tex").write_text(build_main_tex(len(generated_qs), meta['paper_title']))

    for q in generated_qs:
        (job_dir / "questions" / f"q{q.number}.tex").write_text(q.tex_content)

    print(f"  ✅ LaTeX project assembled — {len(generated_qs)} questions")


# ============================================================================
# SECTION 3 — AI PLANNING PASS
# ============================================================================

def plan_worksheet(
    student_profile: dict,
    topic:           str,
    client:          genai.Client,
) -> WorksheetPlan:
    """
    First AI pass: plan the worksheet structure.
    Decides number of questions, difficulty distribution, and question types
    based on the student profile.
    """
    print("  🧠 Planning worksheet structure...")

    year         = student_profile.get("year", 10)
    weaknesses   = student_profile.get("known_misconceptions", [])
    topic_scores = student_profile.get("topic_scores", {})
    error_patterns = student_profile.get("error_patterns", [])
    notes        = student_profile.get("teacher_notes", "")

    topic_score  = topic_scores.get(topic, None)
    score_note   = f"Current score on {topic}: {topic_score:.0%}" if topic_score else "No prior data on this topic."

    prompt = f"""
    You are a highly experienced mathematics tutor designing a personalised practice worksheet.

    STUDENT PROFILE:
    - Year / Grade: {year}
    - Topic for this worksheet: {topic}
    - {score_note}
    - Known misconceptions: {', '.join(weaknesses) if weaknesses else 'None recorded yet'}
    - Common error patterns: {', '.join(error_patterns) if error_patterns else 'None recorded yet'}
    - Teacher notes: {notes or 'None'}

    WORKSHEET PLANNING RULES:
    - Minimum 12 questions. For Year 10+, aim for 14-18 questions.
    - Start with 2-3 confidence-building easy questions.
    - Build up through medium questions that test core method.
    - Include 2-4 challenging questions that stretch the student.
    - Include multi-part questions for complex topics.
    - If the student has known misconceptions, design specific questions that target and expose those misconceptions.
    - Vary question types: diagrams, tables, word problems, show-that, algebraic manipulation.
    - Total marks should be proportional to difficulty — typically 40-70 marks.
    - Harder questions for higher years (Year 11+ should see more 4-5 mark questions).

    Plan the complete worksheet. For each question, state:
    - The specific aspect of {topic} being tested
    - Difficulty (easy/medium/hard)
    - Question type
    - Marks
    - Whether it needs a diagram
    - Which misconception it targets (if any)
    """

    def _call():
        return client.models.generate_content(
            model=MODEL,
            contents=[prompt],
            config=types.GenerateContentConfig(
                response_mime_type="application/json",
                response_schema=WorksheetPlan,
                temperature=0.7,      # some creativity in planning
                max_output_tokens=4096,
            ),
        )

    from pipeline import call_with_retry
    response = call_with_retry(_call)
    plan     = WorksheetPlan.model_validate_json(response.text)
    print(f"  ✅ Plan: {plan.num_questions} questions, ~{plan.total_marks_estimate} marks")
    print(f"     Distribution: {plan.difficulty_distribution}")
    return plan


# ============================================================================
# SECTION 4 — AI QUESTION GENERATION PASS
# ============================================================================

def generate_question(
    q_plan:          QuestionPlan,
    topic:           str,
    student_profile: dict,
    tikz_samples:    str,
    client:          genai.Client,
    prev_questions:  List[str] = None,
) -> GeneratedQuestion:
    """
    Generate one question as complete LaTeX + mark scheme.
    """
    prev_questions = prev_questions or []
    year = student_profile.get("year", 10)

    # Include what questions have already been generated to avoid repetition
    prev_context = ""
    if prev_questions:
        prev_context = f"""
QUESTIONS ALREADY GENERATED (do NOT repeat similar numbers or scenarios):
{chr(10).join(f'Q{i+1}: {q}' for i, q in enumerate(prev_questions))}
"""

    prompt = f"""
You are an expert IGCSE Mathematics question writer.
Generate Question {q_plan.number} for a Year {year} student.

{MACRO_REFERENCE}

{SAMPLE_QUESTIONS}

{"TIKZ DIAGRAM REFERENCE EXAMPLES:" + tikz_samples if tikz_samples and q_plan.has_diagram else ""}

{prev_context}

QUESTION SPECIFICATION:
- Topic aspect: {q_plan.topic_aspect}
- Difficulty: {q_plan.difficulty}
- Type: {q_plan.question_type}
- Marks: {q_plan.marks}
- Needs diagram: {q_plan.has_diagram}
- Misconception to target: {q_plan.misconception_targeted or 'None — general practice'}
- Notes: {q_plan.notes or 'None'}

REQUIREMENTS:
1. Write the complete LaTeX for this question — everything that goes inside a question .tex file.
2. The content must compile within the macro system described above.
3. Use ONLY the macros listed in AVAILABLE LATEX MACROS.
4. Include generous \vspace{{Xcm}} for working space.
5. If the question has a diagram, write complete tikzpicture code.
   Study the tikz examples provided and follow their style closely.
6. Make the question realistic, with clean numbers where possible.
7. For multi-part questions, use \begin{{enumerate}} with \item[(a)] etc. and \partq is available too.
8. Also provide: the correct answer, and a mark scheme in Edexcel format (M1 for..., A1 for...).

OUTPUT the complete tex_content, marks, answer, mark_scheme, and topic_aspect.
tex_content must be valid LaTeX that starts with \begin{{question}} and ends with \end{{question}}.
"""

    def _call():
        return client.models.generate_content(
            model=MODEL,
            contents=[prompt],
            config=types.GenerateContentConfig(
                response_mime_type="application/json",
                response_schema=GeneratedQuestion,
                temperature=0.4,
                max_output_tokens=4096,
            ),
        )

    from pipeline import call_with_retry
    response = call_with_retry(_call)
    q        = GeneratedQuestion.model_validate_json(response.text)
    q.number = q_plan.number  # ensure number matches plan
    return q


# ============================================================================
# SECTION 5 — LATEX COMPILE + FIX LOOP
# ============================================================================

def compile_latex(job_dir: Path) -> tuple[bool, str]:
    """
    Run pdflatex in the job directory.
    Returns (success: bool, error_log: str)
    """
    try:
        result = subprocess.run(
            ["pdflatex", "-interaction=nonstopmode", "-halt-on-error", "main.tex"],
            cwd=job_dir,
            capture_output=True,
            text=True,
            timeout=120,
        )
        # Run twice for totcount/references to resolve
        if result.returncode == 0:
            subprocess.run(
                ["pdflatex", "-interaction=nonstopmode", "main.tex"],
                cwd=job_dir,
                capture_output=True,
                text=True,
                timeout=120,
            )
            return True, ""

        # Extract meaningful errors from log
        log = result.stdout + result.stderr
        errors = extract_latex_errors(log)
        return False, errors

    except subprocess.TimeoutExpired:
        return False, "Compilation timed out after 120 seconds"
    except FileNotFoundError:
        return False, "pdflatex not found — run: sudo apt-get install texlive-latex-extra"


def extract_latex_errors(log: str) -> str:
    """Extract the key error lines from a pdflatex log."""
    error_lines = []
    lines       = log.split("\n")
    for i, line in enumerate(lines):
        if line.startswith("!") or "Error" in line or "Undefined" in line:
            # Include a few lines of context around each error
            context = lines[max(0, i-1):min(len(lines), i+5)]
            error_lines.extend(context)
            error_lines.append("---")
    return "\n".join(error_lines[:50]) if error_lines else log[-2000:]


def fix_question_tex(
    q_number:      int,
    bad_tex:       str,
    error_log:     str,
    client:        genai.Client,
) -> str:
    """Ask AI to fix a specific question's LaTeX based on the compile error."""
    prompt = f"""
The LaTeX for Question {q_number} caused a compilation error.

COMPILE ERROR:
{error_log}

BROKEN TEX CONTENT:
{bad_tex}

{MACRO_REFERENCE}

Fix the LaTeX so it compiles correctly.
Return ONLY the corrected LaTeX content for this question
(starting with \\begin{{question}} and ending with \\end{{question}}).
Do not include any explanation — only the fixed LaTeX.
"""

    def _call():
        return client.models.generate_content(
            model=MODEL,
            contents=[prompt],
            config=types.GenerateContentConfig(
                temperature=0.1,
                max_output_tokens=4096,
            ),
        )

    from pipeline import call_with_retry
    response = call_with_retry(_call)
    fixed    = response.text.strip()

    # Clean up if AI wrapped it in code fences
    fixed = re.sub(r"^```(?:latex)?\n?", "", fixed)
    fixed = re.sub(r"\n?```$", "", fixed)
    return fixed.strip()


def compile_with_fix_loop(
    job_dir:    Path,
    questions:  List[GeneratedQuestion],
    client:     genai.Client,
) -> bool:
    """
    Try to compile. If it fails, identify which question caused the error
    and ask AI to fix it. Retry up to MAX_RETRIES times.
    """
    for attempt in range(MAX_RETRIES):
        print(f"  🔨 Compile attempt {attempt + 1}/{MAX_RETRIES}...")
        success, error_log = compile_latex(job_dir)

        if success:
            print(f"  ✅ Compiled successfully")
            return True

        print(f"  ❌ Compile failed:\n{error_log[:300]}")

        if attempt == MAX_RETRIES - 1:
            print(f"  ❌ All {MAX_RETRIES} compile attempts failed")
            return False

        # Try to identify which question file caused the error
        # and fix it
        fixed_any = False
        for q in questions:
            q_file = job_dir / "questions" / f"q{q.number}.tex"
            q_tex  = q_file.read_text()

            # Heuristic: if question content appears near the error, fix this one
            print(f"  🔧 Attempting to fix Q{q.number}...")
            fixed_tex = fix_question_tex(q.number, q_tex, error_log, client)

            if fixed_tex and fixed_tex != q_tex:
                q_file.write_text(fixed_tex)
                q.tex_content = fixed_tex
                fixed_any     = True
                print(f"  🔧 Q{q.number} rewritten")
                break   # try compiling again after fixing one question

        if not fixed_any:
            print("  ⚠️  Could not identify which question to fix — retrying as-is")

    return False


# ============================================================================
# SECTION 6 — MARK SCHEME ASSEMBLER
# ============================================================================

def build_mark_scheme(
    student_name:   str,
    topic:          str,
    worksheet_plan: WorksheetPlan,
    questions:      List[GeneratedQuestion],
) -> dict:
    """
    Build a structured mark scheme JSON compatible with the AI grader.
    This is passed directly to the grader pipeline.
    """
    return {
        "paper_title":    f"{topic} Worksheet — {student_name}",
        "subject":        "Mathematics",
        "generated_date": date.today().isoformat(),
        "total_marks":    sum(q.marks for q in questions),
        "questions": [
            {
                "question_number": q.number,
                "topic_aspect":    q.topic_aspect,
                "total_marks":     q.marks,
                "correct_answer":  q.answer,
                "mark_scheme":     q.mark_scheme,
                "misconception_targeted": worksheet_plan.questions[q.number - 1].misconception_targeted,
            }
            for q in questions
        ]
    }


# ============================================================================
# SECTION 7 — MAIN ENTRY POINT
# ============================================================================

def generate_worksheet(
    student_profile:  dict,
    topic:            str,
    tikz_samples_dir: Optional[Path] = None,
    output_dir:       Optional[Path]  = None,
    board:            str             = "Cloud LearnX",
    subject:          str             = "Mathematics",
) -> dict:
    """
    Full worksheet generation pipeline.

    Args:
        student_profile:  dict with keys: name, year, topic_scores,
                          known_misconceptions, error_patterns, teacher_notes
        topic:            e.g. "Trigonometry", "Quadratic Equations"
        tikz_samples_dir: path to folder of .tex tikz example files
        output_dir:       where to save the PDF (defaults to worksheet_jobs/)

    Returns:
        {
            "pdf_path":       path to generated PDF,
            "mark_scheme":    dict (for AI grader),
            "plan":           WorksheetPlan dict,
            "job_id":         str,
            "success":        bool,
            "error":          str or None,
        }
    """
    job_id   = str(uuid.uuid4())[:8]
    job_dir  = JOBS_DIR / job_id
    job_dir.mkdir(parents=True)

    student_name = student_profile.get("name", "Student")
    year         = student_profile.get("year", 10)

    print(f"\n{'='*60}")
    print(f"  WORKSHEET GENERATOR — Job {job_id}")
    print(f"  Student: {student_name} | Year {year} | Topic: {topic}")
    print(f"{'='*60}")

    client = genai.Client()

    # Load tikz samples if provided
    tikz_samples = ""
    if tikz_samples_dir and Path(tikz_samples_dir).exists():
        samples = list(Path(tikz_samples_dir).glob("*.tex"))[:15]  # cap at 15
        for s in samples:
            tikz_samples += f"\n--- {s.stem} ---\n{s.read_text()}\n"
        print(f"  📐 Loaded {len(samples)} tikz sample(s) as context")

    try:
        # ── Pass 1: Plan ─────────────────────────────────────────────────
        plan = plan_worksheet(student_profile, topic, client)

        # ── Pass 2: Generate each question ───────────────────────────────
        questions    = []
        prev_summaries = []

        print(f"\n  Generating {plan.num_questions} questions...")
        for q_plan in plan.questions:
            print(f"  → Q{q_plan.number}: {q_plan.topic_aspect} [{q_plan.difficulty}, {q_plan.marks}m]")
            try:
                q = generate_question(
                    q_plan, topic, student_profile,
                    tikz_samples if q_plan.has_diagram else "",
                    client, prev_summaries
                )
                questions.append(q)
                prev_summaries.append(q_plan.topic_aspect)
            except Exception as e:
                print(f"    ⚠️  Q{q_plan.number} generation failed: {e} — skipping")
                traceback.print_exc()

        if not questions:
            raise RuntimeError("No questions were generated successfully.")

        # ── Assemble LaTeX project ────────────────────────────────────────
        meta = {
            "date":        date.today().strftime("%d %B %Y"),
            "board":       board,
            "subject":     subject,
            "paper_title": f"{topic} — Practice Worksheet",
            "tier":        topic,
            "code":        f"Y{year}",
            "time":        f"{len(questions) * 5} minutes (approx.)",
        }

        print(f"\n  📄 Assembling LaTeX project...")
        assemble_latex_project(job_dir, meta, student_name, questions, plan)

        # ── Compile with fix loop ─────────────────────────────────────────
        print(f"\n  🔨 Compiling PDF...")
        compiled = compile_with_fix_loop(job_dir, questions, client)

        pdf_path = job_dir / "main.pdf"

        if not compiled or not pdf_path.exists():
            raise RuntimeError("PDF compilation failed after all retry attempts.")

        # ── Build mark scheme ─────────────────────────────────────────────
        mark_scheme = build_mark_scheme(student_name, topic, plan, questions)

        # Save mark scheme JSON alongside the PDF
        ms_path = job_dir / "mark_scheme.json"
        ms_path.write_text(json.dumps(mark_scheme, indent=2))

        # Copy to output dir if specified
        if output_dir:
            Path(output_dir).mkdir(parents=True, exist_ok=True)
            safe_name = f"{student_name.replace(' ', '_')}_{topic.replace(' ', '_')}_{job_id}"
            final_pdf = Path(output_dir) / f"{safe_name}.pdf"
            final_ms  = Path(output_dir) / f"{safe_name}_markscheme.json"
            shutil.copy(pdf_path, final_pdf)
            shutil.copy(ms_path, final_ms)
            pdf_path = final_pdf

        print(f"\n  🎉 Worksheet complete!")
        print(f"     PDF:         {pdf_path}")
        print(f"     Mark scheme: {ms_path}")
        print(f"     Questions:   {len(questions)}")
        print(f"     Total marks: {sum(q.marks for q in questions)}")

        return {
            "pdf_path":    str(pdf_path),
            "mark_scheme": mark_scheme,
            "plan":        plan.model_dump(),
            "job_id":      job_id,
            "success":     True,
            "error":       None,
        }

    except Exception as e:
        print(f"\n  ❌ Worksheet generation failed: {e}")
        traceback.print_exc()
        return {
            "pdf_path":    None,
            "mark_scheme": None,
            "plan":        None,
            "job_id":      job_id,
            "success":     False,
            "error":       str(e),
        }


# ============================================================================
# ENTRY POINT — Quick Test
# ============================================================================

if __name__ == "__main__":
    test_profile = {
        "name":   "Ahmed Hassan",
        "year":   10,
        "topic_scores": {
            "Trigonometry": 0.45,
            "Pythagoras":   0.70,
        },
        "known_misconceptions": [
            "confuses sin and cos when angle is given",
            "forgets to square root at the end of Pythagoras",
        ],
        "error_patterns": [
            "sign errors in final step",
            "does not show sufficient working for method marks",
        ],
        "teacher_notes": "Strong visual learner. Responds well to diagram-based questions.",
    }

    result = generate_worksheet(
        student_profile  = test_profile,
        topic            = "Trigonometry",
        tikz_samples_dir = Path("tikz_samples"),
        output_dir       = Path("output_worksheets"),
    )

    if result["success"]:
        print(f"\n✅ PDF ready: {result['pdf_path']}")
    else:
        print(f"\n❌ Failed: {result['error']}")
